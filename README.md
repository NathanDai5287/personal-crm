...
compacted timeline of your conversation — written and maintained by an LLM. You
...
- **Compacts** the raw conversation into a `## Timeline` at decreasing resolution
  (recent raw → daily → weekly → older), so a profile stays readable as history
  grows without losing the shape of the relationship.
...
Ingest and compact write **different halves** of a profile and are independent
passes. Full detail on every script is in [`docs/SCRIPTS.md`](docs/SCRIPTS.md);
...
| `node scripts/crm-daily.js` | Full run: ingest + compact everyone | yes |
...
date logic, the merge/compact/eval scripts — is portable Node and runs anywhere.
...
- The only data that leaves the machine is what you send to the model provider,
  and only during the merge/compact/todo steps. Choose the provider accordingly.
...
```
scripts/   runnable jobs (archive, daily, merge, compact, web, backup, …)
