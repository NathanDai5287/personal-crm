'use strict';
// lib/timeline-prompt.js — build the Timeline-step prompt from a template file.
//
// WHY THIS EXISTS: the Timeline prompt used to be a string literal inside
// scripts/crm-timeline.js. That made it the only model instruction in the
// pipeline that could not be versioned, diffed, or A/B'd — and it is arguably
// the higher-stakes of the two, because the Timeline step is a ONE-WAY RATCHET:
// its output replaces raw message lines that are then dropped from the profile. A
...
// supported key today is `system:` — the Timeline prompt historically had NO
...
// `slots` defaults to the Timeline placeholders so existing callers are
// unchanged, but is overridable: prompts/tasks.md uses the same template mechanism
// with different slot names, and hard-coding SLOTS made it throw "missing value for
// {{PERIOD_SENTENCE}}" on a template that never mentions it.
function render(template, vals, slots = SLOTS) {
  const { system, body } = parseTemplate(template);
  let out = body;
  for (const k of slots) {
    if (!(k in vals)) throw new Error(`prompt template: missing value for {{${k}}}`);
    out = out.split(`{{${k}}}`).join(vals[k]);
  }
  const leftover = out.match(/\{\{([A-Z_]+)\}\}/);
  if (leftover) throw new Error(`timeline prompt: unknown placeholder {{${leftover[1]}}}`);
