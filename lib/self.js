...
function selfConversations(cdb, sdb, nameMap = new Map()) {
  const rows = cdb.prepare(
    'SELECT conv_id AS convId, MAX(conversation) AS conversation FROM messages WHERE conv_id IS NOT NULL GROUP BY conv_id ORDER BY conv_id',
  ).all();
  const meta = new Map();
  if (sdb && rows.length) {
    try {
  ...
  for (const r of rows) {
    ...
    let name;
    if (isDm) name = (m && nameMap.get(m.serviceId)) || archived.replace(/^DM with /, '') || 'unknown';
