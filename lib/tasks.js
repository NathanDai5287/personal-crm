...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  importance INTEGER NOT NULL DEFAULT 2,   -- 3 blocked/costly · 2 ordinary · 1 minor
...
// 3 = dropping it is a real problem (someone blocked, money, a date that matters)
// 2 = ordinary obligation, recoverable if it slips
// 1 = minor but genuinely asked-and-agreed
// Out-of-range coerces to 2 rather than throwing: an odd importance is a bad sort key,
// not a reason to lose the task.
...
function insertDraft(db, t) {
  ...
  db.prepare(`
    INSERT INTO tasks (key, slug, contact_name, status, owner, title, description,
                       deadline, confidence, importance, source_msg_id, created_at, updated_at)
    VALUES (?,?,?,'draft',?,?,?,?,?,?,?,?,?)
  `).run(
...
module.exports = {
  ...
  getTask, setStatus, updateTask, addManual, normImportance, OWNERS, STATUSES,
};
