...
// IMAGE CAPTIONING (moondream via Ollama). A local VLM describes an image's SCENE
// (what OCR can't: "two people at a restaurant"), complementing tesseract's text.
// Run through Ollama's HTTP API (it serves the GGUF-quantized model on CPU). q8 is
// the quality-that-fits pick for minmus; override the tag if you pull another.
// A missing/unreachable Ollama just leaves caption rows pending, exactly like a
// missing tesseract/whisper — degradation, never failure.
const CRM_OLLAMA_URL = process.env.CRM_OLLAMA_URL || 'http://127.0.0.1:11434';
const CRM_CAPTION_MODEL = process.env.CRM_CAPTION_MODEL || 'moondream:1.8b-v2-q8_0';
// VIDEO: sample one frame every N seconds, capped, then treat each frame as a still
// image (OCR + caption). Keeps a long video from exploding into hundreds of jobs.
// A positive INTEGER only — a negative/fractional/Infinity/0 value would be
// interpolated straight into the ffmpeg `fps=1/N`/`-frames:v N` args and produce a
// broken filter (→ 0 frames → every video permanently skipped), so a bad override
// falls back to the default rather than corrupting the sampler. (`Number('')` is NaN,
// `Number('  ')` is 0 — both rejected here.)
const _posInt = (v, d) => { const n = Number(v); return Number.isFinite(n) && n >= 1 ? Math.floor(n) : d; };
const CRM_VIDEO_FRAME_SEC = _posInt(process.env.CRM_VIDEO_FRAME_SEC, 10);
const CRM_VIDEO_MAX_FRAMES = _posInt(process.env.CRM_VIDEO_MAX_FRAMES, 12);
...
// NATHAN'S OWN PROFILE (lib/self.js): the same pipeline as a contact, inverted — its
// own merge stream over every archived conversation, with its own prompts. The slug is
// the reserved owner slug everything else already uses for Nathan (nicknames, graph).
const SELF_SLUG = 'nathan';
const SELF_MERGE_PROMPT = path.posix.join(ROOT, 'prompts', 'self-merge.md');
const SELF_TIMELINE_PROMPT = process.env.CRM_SELF_TIMELINE_PROMPT || path.posix.join(ROOT, 'prompts', 'self-compact.md');
...
// ---- web auth: Google Sign-In + per-guest isolation ---------------------------
// The web app authenticates with Google (lib/auth-google) and hands out signed
// session cookies (lib/session). The owner (ADMIN_EMAIL) gets the full app; any
// other Google account whose email uniquely matches a contact sees only that one
// page (lib/identity + lib/lens). PUBLIC_ORIGIN is the https URL the app is
// reached at (through the cloudflared tunnel) — used to build the OAuth
// redirect_uri and to mark cookies Secure, so we never trust proxy headers.
const ADMIN_EMAIL = (process.env.CRM_ADMIN_EMAIL || 'nathandai2000@gmail.com').toLowerCase();
// Validate the public origin: it drives the OAuth redirect_uri AND whether session
// cookies are marked Secure (crm-web derives COOKIE_SECURE from its scheme). An
// invalid, path-bearing, or non-local http origin would silently downgrade cookies,
// so reject those and fall SAFE to the secure default rather than trust the string.
const PUBLIC_ORIGIN = (() => {
  const raw = (process.env.CRM_PUBLIC_ORIGIN || 'https://crm.cal.taxi').replace(/\/+$/, '');
  try {
    const u = new URL(raw);
    const isLocal = /^(localhost|127\.0\.0\.1|\[?::1\]?)$/i.test(u.hostname);
    const pathOk = u.pathname === '/' || u.pathname === '';
    if (pathOk && (u.protocol === 'https:' || (u.protocol === 'http:' && isLocal))) return raw;
  } catch { /* not a URL */ }
  console.warn(`WARNING: CRM_PUBLIC_ORIGIN="${raw}" is not a valid https origin (non-local http, a path, or malformed). Falling back to https://crm.cal.taxi so session cookies stay Secure.`);
  return 'https://crm.cal.taxi';
})();
// Session HMAC key. Env wins; else data/session-secret.txt (auto-generated 0600
// by the web app on first start). Rotating it invalidates all sessions.
const SESSION_SECRET_FILE = path.posix.join(DATA_DIR, 'session-secret.txt');
// Google OAuth client credentials. Env wins; else data/oauth.json (0600):
//   { "client_id": "...", "client_secret": "..." }
const OAUTH_FILE = path.posix.join(DATA_DIR, 'oauth.json');
function loadOauth() {
  const envId = process.env.GOOGLE_CLIENT_ID;
  const envSecret = process.env.GOOGLE_CLIENT_SECRET;
  if (envId && envSecret) return { clientId: envId, clientSecret: envSecret };
  try {
    const j = JSON.parse(fs.readFileSync(OAUTH_FILE, 'utf8'));
    if (j && j.client_id && j.client_secret) return { clientId: j.client_id, clientSecret: j.client_secret };
  } catch { /* not configured on this machine */ }
  return { clientId: envId || null, clientSecret: envSecret || null };
}
