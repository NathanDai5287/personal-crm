...
function acquire(name) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      // ATOMIC, ALREADY-POPULATED create. The old form — openSync('wx') then a separate
      // writeSync — left a window in which the lock file existed but was still EMPTY; a
      // second acquirer hitting EEXIST in that window read '', JSON.parse threw,
      // readHolder returned null, isStale(null) said "abandoned", and it STOLE a live
      // lock — two holders at once. Writing a fully-formed temp file and hard-linking it
      // into place makes the lock file appear complete the instant it exists, so it is
      // never observed empty. (Same filesystem — both live in DATA_DIR — so link is atomic.)
      const payload = JSON.stringify({ pid: process.pid, name: name || 'pipeline', startedAt: Date.now() });
      const tmp = `${LOCK_FILE}.tmp-${process.pid}-${Date.now()}`;
      fs.writeFileSync(tmp, payload);
      try { fs.linkSync(tmp, LOCK_FILE); } // O_EXCL-equivalent: throws EEXIST if held
