...
const { SELF_SLUG } = require('./config');
...
function validateFact(raw, slug, messages, runId, validMessageIds) {
  ...
  // NATHAN'S OWN PROFILE never stores an email: email is the key a guest's Google sign-in
  // is matched on (lib/identity), so an address of his here could hand a guest his page.
  // Dropped, not thrown — the self-merge prompt may still emit one.
  if (slug === SELF_SLUG && field === 'email') return null;
