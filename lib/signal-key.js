...
require('./env'); // load .env so CRM_SIGNAL_* overrides are visible below

// Windows-only paths — this whole module is DPAPI-based and does not run on
// macOS/Linux (there Signal's key lives in the OS keychain). Overridable via
// .env for a non-default Signal install; on other platforms, skip rederivation
// entirely by seeding data/signal-key.txt (SIGNAL_KEY_FALLBACK).
const CONFIG_PATH = process.env.CRM_SIGNAL_CONFIG || 'C:/Users/natha/AppData/Roaming/Signal/config.json';
const LOCAL_STATE_PATH = process.env.CRM_SIGNAL_LOCAL_STATE || 'C:/Users/natha/AppData/Roaming/Signal/Local State';
