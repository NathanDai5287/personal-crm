...
//
// SOURCES today:
//   contacts/<slug>.md header  → name, signalId, phone, email, relationship, birthday, first/last
...
const { CONTACTS_DIR, SELF_SLUG } = require('./config');
const { dateKey } = require('./weeks');
const { currentFacts, neighbors } = require('./schema');

const PERSON_FIELDS = {
  signal_id: 'signalId', phone: 'phone', email: 'email', relationship: 'relationship',
  birthday: 'birthday', first_contact: 'firstContact', last_contact: 'lastContact',
};
...
function parseHeader(md) {
  ...
  for (const line of String(md).split(/\r?\n/)) {
    ...
    // Strip markup only when it WRAPS THE WHOLE value (`_unknown_`, `*bold*`) so
    // placeholder detection in real() still fires — but never internal underscores,
    // which are legal in an email local part (foo_bar@…) and mid-value italics
    // (`_close_ friend`), both of which the old blanket strip silently corrupted.
    if (m) {
      let val = m[2].trim();
      const wrap = /^([_*])(.+)\1$/.exec(val); // a single _..._ or *...* around the entire value
      meta[m[1].trim().toLowerCase()] = (wrap ? wrap[2] : val).trim();
    }
  }
  return meta;
}
...
function allPeople(opts = {}) {
  ...
  // Nathan's own profile (lib/self.js) shares the directory but is not a contact.
  if (!opts.includeSelf) files = files.filter((f) => f !== `${SELF_SLUG}.md`);
  return files
