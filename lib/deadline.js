...
function onWeekday(now, target, forceNext) {
...
function onMonthDay(now, monthIdx, day) {
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  let d = new Date(now.getFullYear(), monthIdx, day);
  if (d < today) d = new Date(now.getFullYear() + 1, monthIdx, day);
  return d;
}
...
function parseDeadline(input, now) {
  ...
  if (s === 'today' || s === 'eod' || s === 'tonight' || s === 'now') return iso(now);
  if (s === 'tomorrow' || s === 'tmr' || s === 'tmrw' || s === 'tom') return iso(addDays(now, 1));
  if (s === 'yesterday') return iso(addDays(now, -1));
  if (s === 'next week') return iso(addDays(now, 7));
  if (s === 'eow' || s === 'end of week') return iso(onWeekday(now, 5, false));

  // "in 3 days" / "3 days" / "3d" / "in 2 weeks" / "2w"
  let m = s.match(/^(?:in\s+)?(\d+)\s*(d|days?|w|weeks?)$/);
  if (m) return iso(addDays(now, Number(m[1]) * (m[2][0] === 'w' ? 7 : 1)));

  // weekday name, optionally "next"
  m = s.match(/^(next\s+)?([a-z]+)$/);
  if (m && Object.prototype.hasOwnProperty.call(WEEKDAYS, m[2])) {
    return iso(onWeekday(now, WEEKDAYS[m[2]], Boolean(m[1])));
  }

  // "aug 15" / "august 15th"
  m = s.match(/^([a-z]{3,9})\.?\s+(\d{1,2})(?:st|nd|rd|th)?$/);
  if (m && Object.prototype.hasOwnProperty.call(MONTHS, m[1])) {
    return iso(onMonthDay(now, MONTHS[m[1]], Number(m[2])));
  }
  // "15 aug" / "15th august"
  m = s.match(/^(\d{1,2})(?:st|nd|rd|th)?\s+([a-z]{3,9})\.?$/);
  if (m && Object.prototype.hasOwnProperty.call(MONTHS, m[2])) {
    return iso(onMonthDay(now, MONTHS[m[2]], Number(m[1])));
  }
  // "8/15" (month/day)
  m = s.match(/^(\d{1,2})\/(\d{1,2})$/);
  if (m) return iso(onMonthDay(now, Number(m[1]) - 1, Number(m[2])));
