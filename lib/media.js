...
// OCR text and transcripts are UNTRUSTED content folded into the merge/Timeline
// ledger — a photo of text or a dictated voice note could carry the ledger's own
...
// merge. Strip those so it can only read as plain quoted content, then run the same
// PII redaction every message body gets (the fold otherwise bypassed redact() — the
// consensus top finding across P3/P4).
function sanitizeFold(s) {
  return redact(String(s == null ? '' : s).replace(/[⟨⟩[\]|#"]/g, ' ').replace(/\s+/g, ' ').trim());
}
