...
function ensureMediaTable(cdb) {
  cdb.exec(`
    CREATE TABLE IF NOT EXISTS media_text (
      ...
      attempts     INTEGER NOT NULL DEFAULT 0,  -- failed tries so far (see noteFailure)
      next_attempt_at INTEGER,         -- don't re-claim before this (retry backoff); NULL = now
      created_at   INTEGER,
      updated_at   INTEGER,
  ...
  // Retry-bookkeeping columns, added after the fact. attempts + next_attempt_at let a
  // TRANSIENT failure (Ollama connection blip, timeout, a momentary db lock) return to
  // 'pending' with a backoff instead of a terminal 'error', so the queue self-heals
  // across sweeps without anyone running --retry-errors. ADD COLUMN is cheap; guarded so
  // it only runs once. Re-read cols because the migration above may have rebuilt the table.
  let cols2 = [];
  try { cols2 = cdb.prepare('PRAGMA table_info(media_text)').all().map((c) => c.name); } catch { cols2 = []; }
  if (cols2.length && !cols2.includes('attempts')) cdb.exec('ALTER TABLE media_text ADD COLUMN attempts INTEGER NOT NULL DEFAULT 0');
  if (cols2.length && !cols2.includes('next_attempt_at')) cdb.exec('ALTER TABLE media_text ADD COLUMN next_attempt_at INTEGER');
}
...
// Backoff schedule for a TRANSIENT failure, one entry per attempt. The gaps are short
// enough that a long drain re-tries within the same run (the while-loop keeps claiming
// other rows meanwhile), and an hourly sweep re-tries anything left — so a passing
// Ollama/network blip recovers on its own. Length also caps the attempts.
const RETRY_BACKOFF_MS = [30_000, 120_000, 600_000, 2_700_000, 10_800_000]; // 30s, 2m, 10m, 45m, 3h
const MAX_ATTEMPTS = RETRY_BACKOFF_MS.length;

// A failure that is probably transient: bump attempts and, while attempts remain, send
// the row back to 'pending' with a backoff (next_attempt_at) instead of a terminal
// 'error'. Only once the schedule is exhausted does it become a real 'error'. This is
// what makes the queue self-heal unattended. Returns { retrying, attempts } for logging.
// The error text is kept on the row either way, so a still-retrying row shows its last
// failure; the merge gate (hasPendingMedia) keeps holding the message while it's pending,
// so a caption is never lost to a momentary blip.
function noteFailure(cdb, key, error, engine, now = Date.now()) {
  const cur = cdb.prepare('SELECT attempts FROM media_text WHERE hash = ? AND kind = ? AND part = ?')
    .get(key.hash, key.kind, key.part || 0);
  const attempts = ((cur && cur.attempts) || 0) + 1;
  const errText = String(error).slice(0, 500);
  if (attempts >= MAX_ATTEMPTS) {
    cdb.prepare("UPDATE media_text SET status = 'error', error = ?, engine = ?, attempts = ?, next_attempt_at = NULL, updated_at = ? WHERE hash = ? AND kind = ? AND part = ?")
      .run(errText, engine || null, attempts, now, key.hash, key.kind, key.part || 0);
    return { retrying: false, attempts };
  }
  const next = now + RETRY_BACKOFF_MS[attempts - 1];
  cdb.prepare("UPDATE media_text SET status = 'pending', error = ?, engine = ?, attempts = ?, next_attempt_at = ?, updated_at = ? WHERE hash = ? AND kind = ? AND part = ?")
    .run(errText, engine || null, attempts, next, now, key.hash, key.kind, key.part || 0);
  return { retrying: true, attempts };
}
...
function requeueErrors(cdb, now = Date.now()) {
  return cdb.prepare("UPDATE media_text SET status = 'pending', error = NULL, attempts = 0, next_attempt_at = NULL, updated_at = ? WHERE status = 'error'").run(now).changes || 0;
}
...
// Does any attachment on this message still have media work IN FLIGHT (pending or
// processing)? Used by the merge planner to HOLD a message whose caption/OCR/transcript
// isn't finished yet. Rationale: the merge folds media text via foldSuffix, which reads
// only status='done' rows; if we merged a message before its media finished, it would
// enter the `merged` frontier without that text and never be re-fed — the caption would
// be lost from the profile forever. 'done'/'skip'/'error' are all TERMINAL, so a genuine
// no-op or a permanent failure never blocks (only a live, resolvable job does). Note: a
// hash with NO media_text row at all is not pending — either the attachment type yields
// no jobs, or (impossible in practice, since archive enqueues under the same serialized
// lock that gates a later merge) it hasn't been enqueued yet.
function hasPendingMedia(cdb, hashesStr) {
  if (!hashesStr) return false;
  const hashes = [...new Set(String(hashesStr).split(/\s+/).filter(Boolean))];
  if (!hashes.length) return false;
  const r = cdb.prepare(
    `SELECT 1 FROM media_text
      WHERE status IN ('pending','processing') AND hash IN (${hashes.map(() => '?').join(',')})
      LIMIT 1`,
  ).get(...hashes);
  return Boolean(r);
}

// Health snapshot for the /health page: every terminal-error row, plus rows stuck in
// 'processing' past the stale window (a symptom of a dead worker that requeueStale
// hasn't yet reclaimed), plus the age of the oldest still-pending row (a backlog that
// isn't draining means the worker is down — and, with the merge gate above, that stalls
// merges too). Bounded result sets so the page stays cheap.
function health(cdb, now = Date.now(), staleMs = 30 * 60 * 1000) {
  const errors = cdb.prepare(
    "SELECT hash, kind, part, content_type, error, updated_at FROM media_text WHERE status = 'error' ORDER BY updated_at DESC LIMIT 100",
  ).all();
  const stuck = cdb.prepare(
    "SELECT hash, kind, part, content_type, updated_at FROM media_text WHERE status = 'processing' AND updated_at < ? ORDER BY updated_at ASC LIMIT 100",
  ).all(now - staleMs);
  // Only CLAIMABLE pending rows signal a stalled worker; a row waiting out its retry
  // backoff (next_attempt_at in the future) is working as intended, not a stuck backlog.
  const oldestPending = cdb.prepare(
    "SELECT MIN(created_at) m FROM media_text WHERE status = 'pending' AND (next_attempt_at IS NULL OR next_attempt_at <= ?)",
  ).get(now);
  return {
    errors,
    stuck,
    oldestPendingMs: oldestPending && oldestPending.m ? oldestPending.m : null,
    counts: counts(cdb),
  };
}
