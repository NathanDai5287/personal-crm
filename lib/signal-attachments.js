...
// ALL decryptable rows for a hash, not just one. Identical files share a plaintextHash,
// so a popular image can have many rows — and one row's `path` can point at a blob Signal
// has since purged from disk while a sibling row's blob is still present. Returning every
// candidate lets decryptByHash try the next one instead of 404-ing on the first dead path.
function candidatesByHash(sdb, hash) {
  return sdb.prepare(
    `SELECT path, size, localKey, contentType, duration
       FROM message_attachments
      WHERE plaintextHash = ? AND path IS NOT NULL AND localKey IS NOT NULL`,
  ).all([hash]) || []; // vendor sqlcipher wrapper requires params as an array
}

function decryptableByHash(sdb, hash) {
  return candidatesByHash(sdb, hash)[0] || null;
}
...
function decryptByHash(sdb, hash) {
  const rows = candidatesByHash(sdb, hash);
  if (!rows.length) throw new Error(`no downloaded attachment for hash ${hash}`);
  // Try every candidate: a row whose blob was purged (path set, file gone) or fails its
  // MAC must not 404 the request when an identical sibling row still decrypts. Only if
  // ALL candidates fail do we surface the last error.
  let lastErr = null;
  for (const row of rows) {
    try { return { buf: decryptRow(row), row }; } catch (e) { lastErr = e; }
  }
  throw lastErr || new Error(`no decryptable attachment for hash ${hash}`);
}

module.exports = { decryptRow, decryptByHash, decryptableByHash, candidatesByHash };
