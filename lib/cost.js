...
// One Timeline or todo model call — single-shot, no tools, short out.
function timelineCallUsd(model, opts = {}) {
  const p = priceOf(model);
  if (!p) return null;
  if (p.free) return 0;
...
function estIngestFromRows(mergeModel, timelineModel, rows) {
  ...
  for (const c of chunks) {
    ...
    const cp = timelineCallUsd(timelineModel, { bucketTokens: Math.min(c.tokens, 4_000) });
...
module.exports = {
  ...
  mergeCallUsd, timelineCallUsd, mergeCallSeconds, timelineCallSeconds,
