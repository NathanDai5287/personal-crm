...
// TIME-OF-DAY PRICING (deepseek/*). DeepSeek bills HALF price off-peak. Peak is
// 01:00-04:00 and 06:00-10:00 UTC, Monday-Friday; everything else — weekends
// included — is off-peak. These windows are the provider's billing clock, so they
// are deliberately UTC, the one exception to "Pacific everywhere". pi's catalog
// (and the cost.total pi writes into each session turn) carries only the PEAK rate,
// so without this every deepseek number would read 2x high off-peak. Chinese public
// holidays are also off-peak per DeepSeek; we don't model them, so a holiday run is
// over- not under-estimated.
const DEEPSEEK_PEAK_UTC = [[1, 4], [6, 10]]; // [startHour, endHour) UTC, Mon-Fri
function rateFactor(model, atMs = Date.now()) {
  const m = String(model || '');
  if (!(m === 'deepseek' || m.startsWith('deepseek/'))) return 1;
  const d = new Date(atMs);
  const dow = d.getUTCDay();
  if (dow === 0 || dow === 6) return 0.5;
  const h = d.getUTCHours();
  return DEEPSEEK_PEAK_UTC.some(([a, b]) => h >= a && h < b) ? 1 : 0.5;
}

// { input, output } dollars-per-million at time `atMs` (default now), or null when
// the model isn't in pi's catalog (caller shows "—"). basePriceOf is the raw catalog
// (peak) rate; priceOf applies rateFactor.
function priceOf(model, atMs = Date.now()) {
  const p = basePriceOf(model);
  if (!p) return null;
  const k = rateFactor(model, atMs);
  return k === 1 ? p : { ...p, input: p.input * k, output: p.output * k };
}

let _store = null;
function basePriceOf(model) {
  // Every model is priced from the catalog — anthropic/* included. These ride the
  // Claude subscription rather than metered per-token billing, but pi reports a real
  // dollar cost for them, so we show that number rather than hiding it as "on plan".
...
function fitCostModel() {
  ...
  for (const l of lines) {
    try {
      ...
      // Samples are real billed USD at the time they ran; divide out the time-of-day
      // factor so off-peak and peak samples fit one set of (turns, out) coefficients
      // against the catalog (peak) price.
      if (s && s.usd > 0 && s.base > 0) (byModel[s.model] = byModel[s.model] || []).push({ ...s, usd: s.usd / rateFactor(s.model, s.at || Date.now()) });
    } catch { /* skip bad line */ }
  }
  const fit = {};
...
function mergeCallUsd(model, opts = {}) {
  const p = priceOf(model, opts.atMs);
  if (!p) return null;
  if (p.free) return 0;
...
function sumSessionCostUsd(dir) {
  ...
  for (const f of files) {
    ...
    for (const ln of text.split(/\r?\n/)) {
      ...
      if (u && u.cost && typeof u.cost.total === 'number') {
        // pi prices every turn at the catalog (peak) rate; apply the provider's
        // time-of-day discount using that turn's own timestamp.
        const m = o.message || o;
        const at = typeof m.timestamp === 'number' ? m.timestamp : Date.parse(o.timestamp || '') || Date.now();
        total += u.cost.total * rateFactor(m.provider || m.model, at);
        turns += 1; saw = true;
      }
    }
  }
...
module.exports = {
  CHARS_PER_TOKEN, tok, isFree, priceOf, basePriceOf, rateFactor,
