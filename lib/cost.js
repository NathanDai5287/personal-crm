...
const {
  MERGE_PROMPT, COMPACT_PROMPT, COST_SAMPLES, COST_MODEL,
...
function fitCostModel() {
  ...
  for (const [model, samples] of Object.entries(byModel)) {
    ...
    const A = (n * sxy - sx * sy) / denom; // price_in · turns
    const B = (sy - A * sx) / n;           // price_out · out
    const turns = A / p.input;
    const out = B / p.output;
    // Reject a pathological fit and keep the defaults rather than publish nonsense.
    fit[model] = {
      turns: turns >= 0.5 && turns <= 20 ? Number(turns.toFixed(2)) : MERGE_TURNS,
      out: out >= 0 && out <= 100_000 ? Math.round(out) : MERGE_OUT,
      n,
      updatedAt: Date.now(),
    };
  }
  try { fs.writeFileSync(COST_MODEL, JSON.stringify(fit, null, 2)); } catch { /* non-fatal */ }
  _fit = fit;
  return fit;
}
...
// (an estimate can ignore the rare late-sync inversion). Adds one compact (Timeline)
...
function estIngestFromRows(mergeModel, compactModel, rows) {
  ...
  for (const c of chunks) {
    ...
    const cp = compactCallUsd(compactModel, { bucketTokens: Math.min(c.tokens, 4_000) });
    ...
    seconds += mergeCallSeconds({ ledgerTokens: c.tokens }) + compactCallSeconds();
  }
...
// block of every assistant message, concatenated in order. This is where the
...
// an unreadable/empty dir. Concatenating across every session file in the dir is
// safe — a retried merge leaves several, and the parser downstream scans globally
// and dedups, so a duplicated block costs nothing.
