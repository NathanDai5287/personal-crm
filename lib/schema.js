...
function mentionEdges(db) {
  mentionSchema(db);
  return db.prepare(
    'SELECT from_slug, to_slug, COUNT(*) n, MAX(observed_at) last FROM mentions GROUP BY from_slug, to_slug ORDER BY n DESC'
  ).all();
}
...
function edgeCitations(db, fromSlug, toSlug) {
  mentionSchema(db);
  return db.prepare(`
    ...
    WHERE m.from_slug = ? AND m.to_slug = ?
