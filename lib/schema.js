...
function factSchema(db) {
  ...
  // A short human gloss of what the FIELD means ("their elected role in the Theta Xi
  // chapter"), authored by the model alongside the value. It is not part of any
  // identity/supersession key — it is a disambiguation aid shown back to the model on
  // later chunks so it REUSES an existing field name instead of coining a synonym for
  // the same attribute. Nullable: facts written before this column stay description-less
  // until the field is next observed.
  try { db.exec('ALTER TABLE facts ADD COLUMN description TEXT'); } catch { /* exists */ }
}
...
// The one true birthday format. Two shapes are legal and NOTHING else is:
//   'YYYY-MM-DD'  a full date         (deriveAge can compute an age)
//   '--MM-DD'     month + day, no year ("my birthday's March 14" — year unknown)
// A bare 'MM-DD' is accepted and canonicalised to '--MM-DD' for tolerance. Every
// other input — a real year with a bad month, free text like "March 14", a
// day-of-month a month can't have — returns null, which every caller treats as
// "reject this value" rather than storing a rotting string. Feb 29 is allowed in
// both shapes (a real leap-day birthday; validated against a known leap year).
function normalizeBirthday(v) {
  const s = String(v == null ? '' : v).trim();
  if (!s) return null;
  const dayOk = (mo, d) => {
    if (mo < 1 || mo > 12 || d < 1) return false;
    return d <= [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][mo - 1]; // leap-lenient
  };
  let m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(s);
  if (m) {
    const y = Number(m[1]); const mo = Number(m[2]); const d = Number(m[3]);
    // A full date must be a real calendar date (this rejects Feb 30, and Feb 29 in
    // a common year), and within a sane human range.
    const dt = new Date(Date.UTC(y, mo - 1, d));
    const real = dt.getUTCFullYear() === y && dt.getUTCMonth() === mo - 1 && dt.getUTCDate() === d;
    if (!real || y < 1900 || y > 2100) return null;
    return `${m[1]}-${m[2]}-${m[3]}`;
  }
  m = /^(?:--)?(\d{2})-(\d{2})$/.exec(s);
  if (m) {
    const mo = Number(m[1]); const d = Number(m[2]);
    return dayOk(mo, d) ? `--${m[1]}-${m[2]}` : null;
  }
  return null;
}

// Email: trim, validate a single address of the ordinary local@domain.tld shape,
// lowercase the domain (case-insensitive by spec) while leaving the local part as
// typed. Returns null on anything that isn't a plausible single address, so a
// caller stores a usable value or nothing — never a broken string.
function normalizeEmail(v) {
  const s = String(v == null ? '' : v).trim();
  if (!s || s.length > 254) return null;
  // Reject control and zero-width / format characters outright — they never belong
  // in an address and would otherwise be stored invisibly (e.g. a trailing ZWSP).
  if ([...s].some((c) => { const k = c.codePointAt(0); return k < 0x20 || (k >= 0x7f && k <= 0x9f) || (k >= 0x200b && k <= 0x200f) || k === 0x2028 || k === 0x2029 || k === 0xfeff; })) return null;
  const m = /^([^\s@]+)@([^\s@]+)$/.exec(s);
  if (!m) return null;
  const local = m[1];
  ...
  // domain: two or more dot-separated labels, each alphanumeric with only internal
  // hyphens; the TLD at least two letters. Rejects foo@bar..com, foo@.bar.com,
  // foo@bar.com., foo@-bar.com, a@b.c, etc.
...
function recordFact(db, f) {
  ...
  const incumbent = db.prepare(`
    SELECT id, observed_at, retracted, retracted_at, src_msg FROM facts
  ...
  const staleByTime = Boolean(incumbent && incumbentAt > f.observed_at);

  // THE MODEL MAY NOT OVERRIDE A HUMAN. A manual save records with src_msg NULL
  // (crm-web applyManualEdit); a model-derived fact ALWAYS carries a src_msg
  // (validateFact requires one). So src_msg IS NULL is the reliable "human entered
  // this" marker. When a LIVE (not retracted) human value already holds the field,
  // an arriving model fact is born superseded: kept in history, never crowned, and
  // the human's value is left untouched. Human-over-anything and model-over-model
  // keep the normal latest-wins rule. A field the human has CLEARED (retracted) is
  // open to the model again — clearing set no value, so filling it overrides nothing.
  const protectedByHuman = Boolean(
    incumbent && incumbent.src_msg == null && !incumbent.retracted && f.src_msg != null,
  );
  const stale = staleByTime || protectedByHuman;

  const info = db.prepare(`
    INSERT INTO facts (slug, field, kind, value, value_num, unit, description,
      period_start, period_end, period_label, as_of, as_of_stated,
      src_msg, observed_at, recorded_at, run_id, identity_key, superseded_by)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    f.slug, f.field, f.kind, f.value, f.value_num ?? null, f.unit ?? null, f.description ?? null,
  ...
  return {
    id, superseded: Boolean(incumbent && !stale),
    staleOnArrival: staleByTime, blockedByHuman: protectedByHuman,
  };
}
...
module.exports = {
  FACT_KINDS, DERIVED_FIELDS, deriveAge, normalizeBirthday, normalizeEmail,
