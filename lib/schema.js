...
// trivial case of naming the other party of your own 1:1: a citation whose message is a
// 'DM with …' and whose target is one of that DM's two people (Nathan or the DM contact).
...
function mentionEdges(db, opts = {}) {
  ...
  if (opts.hideDm) {
    ...
    where.push("NOT (COALESCE(msg.conversation,'') LIKE 'DM with %' AND (m.to_slug = 'nathan' OR m.to_slug = msg.contact_slug))");
  }
...
function edgeCitations(db, fromSlug, toSlug, opts = {}) {
  mentionSchema(db);
  const where = ['m.from_slug = ?', 'm.to_slug = ?'];
  if (opts.hideMine) where.push("m.from_slug <> 'nathan'");
  if (opts.hideDm) where.push("NOT (COALESCE(msg.conversation,'') LIKE 'DM with %' AND (m.to_slug = 'nathan' OR m.to_slug = msg.contact_slug))");
  return db.prepare(`
    ...
    ORDER BY m.observed_at DESC, m.src_msg DESC`).all(fromSlug, toSlug);
}
