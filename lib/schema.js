...
function retractCurrentFact(db, slug, field) {
  factSchema(db);
  return db.prepare(`UPDATE facts SET retracted = 1
    WHERE slug = ? AND field = ? AND superseded_by IS NULL AND retracted = 0`).run(slug, field).changes;
}
