...
function mentionEdges(db, opts = {}) {
  ...
  if (opts.hideDm) {
    ...
    where.push("NOT (msg.conversation LIKE 'DM with %' AND (m.to_slug = 'nathan' OR m.to_slug = msg.contact_slug))");
  }
...
function edgeCitations(db, fromSlug, toSlug, opts = {}) {
  ...
  if (opts.hideMine) where.push("m.from_slug <> 'nathan'");
  if (opts.hideDm) where.push("NOT (msg.conversation LIKE 'DM with %' AND (m.to_slug = 'nathan' OR m.to_slug = msg.contact_slug))");
  return db.prepare(`
