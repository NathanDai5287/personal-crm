...
// walk stops at the first one — so no merge sees a partial week. A final check
// flushes a silent backlog that has aged past the gate with no new message to trip
// it. Days are 04:00-Pacific (dayNumber), DST-safe and absolute.
//   lastMergeMs: max sent_at over everything already merged for this contact, or null
//     for a fresh contact (clock starts at the first unmerged row's day). The live
//     path reconstructs the identical value, so a resumed run continues the one-shot
//     backfill exactly.
