...
function mirrorMessages(cdb, items) {
  ...
  // SECONDARY-DEVICE CONTENT DEDUP. When this machine's archive was seeded by
  // copying another device's crm.db, the same message carries a DIFFERENT Signal
  // rowid here, so its id can't identify it against the copied rows. Identity is
  // (sent_at, src): src is the sender's Signal serviceId, IDENTICAL on every
  // linked device, and a sender cannot post two messages in the same millisecond.
  // NOT the display `sender` label — that is derived per-device from Signal
  // profile names and differs across devices for group members, which would let
  // the same group message slip past and duplicate. `src IS ?` is NULL-safe, for
  // outgoing messages that carry a NULL sourceServiceId. Only prepared when an
  // offset is set; a primary device (offset 0) keeps the old path.
  const bySrc = ARCHIVE_ID_OFFSET
    ? cdb.prepare('SELECT 1 AS ok FROM messages WHERE sent_at = ? AND src IS ? LIMIT 1')
    : null;
  let inserted = 0;
  const collisions = [];
  for (const it of items) {
    ...
    if (ARCHIVE_ID_OFFSET) {
      ...
      if (bySrc.get(it.sentAt, it.src ?? null)) continue;
