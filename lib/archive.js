...
function mirrorMessages(cdb, items) {
  ...
  // then (sent_at, sender) — a person cannot send two messages in the same
  // millisecond, and sender labels match once nicknames are shared. Only prepared
  // when an offset is configured; a primary device (offset 0) keeps the old path.
  const bySent = ARCHIVE_ID_OFFSET
    ? cdb.prepare('SELECT 1 AS ok FROM messages WHERE sent_at = ? AND sender = ? LIMIT 1')
    : null;
  let inserted = 0;
  const collisions = [];
  for (const it of items) {
    ...
    if (ARCHIVE_ID_OFFSET) {
      ...
      if (bySent.get(it.sentAt, it.sender)) continue;
