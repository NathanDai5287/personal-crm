...
//   - crm-timeline.js mirrors every message it summarizes into the Timeline
...
function ensureMessagesTable(cdb) {
  cdb.exec(`
    ...
    -- INGEST LEDGER: which archive messages have been merged into which contact's
    -- profile. This is the merge frontier — it REPLACES the old single rowid cursor.
    -- Per (slug, message_id) because one group message can be merged for several
    -- contacts (full group context), and because tracking merges explicitly (rather
    -- than a max-rowid watermark) lets the planner pull the backlog in sent_at order
    -- and pick up a late-synced OLD message in its chronological place instead of
    -- stranding it below an advancing cursor. See AGENTS.md and crm-refresh.
  ...
  // Space-separated plaintextHash(es) of this message's downloaded image/audio
  // attachments — the link to media_text (OCR/transcript), folded into ledger and
  // Timeline lines at read time. Kept on the ROW (not a join table) so it travels
  // with the message and needs no fragile archive-id mapping. NULL = none / legacy.
  try { cdb.exec('ALTER TABLE messages ADD COLUMN att_hashes TEXT'); } catch { /* exists */ }
  // The archive and structured Person tables share crm.db. Initialize the latter
  // alongside the archive schema so read paths never perform surprise DDL.
  require('./schema').ensureSchema(cdb);
}
...
function mirrorMessages(cdb, items) {
  // Ensure the schema BEFORE the empty-input early return: on a fresh crm.db whose
  // very first sweep finds nothing new, returning here without creating the tables
  // left the next `planContact` to throw "no such table: merged" on cold start.
  ensureMessagesTable(cdb);
  if (!items || items.length === 0) return { seen: 0, inserted: 0, collisions: [] };
  const stmt = cdb.prepare(
    'INSERT OR IGNORE INTO messages (id, conv_id, conversation, contact_slug, sent_at, sender, body, src, type, att_hashes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  );
  ...
  // Backfill att_hashes onto a row archived before this feature (or before its
  // attachment finished downloading) when a re-sweep now has the hashes. Additive:
  // only sets it when currently NULL, so a deep sweep lights up OCR/STT for the
  // whole back-catalogue without touching bodies.
  const fillHashes = cdb.prepare('UPDATE messages SET att_hashes = ? WHERE id = ? AND att_hashes IS NULL');
  ...
  // `src` is a SAFE tie-breaker on top of (sent_at, type): sourceServiceId
  // identifies the SENDER (device-stable, identical across linked devices), so two
  // rows with the same instant+type but DIFFERENT non-null senders are provably
  // different messages and must not be deduped into one — that was the residual
  // collision (measured 3 ties in 86k). The guard only fires when BOTH sides carry
  // a src: `src IS NULL OR ? IS NULL` keeps the old (sent_at,type)-only match for
  // the ~19% of legacy rows with a NULL src, so this can never create a NEW
  // duplicate — it only splits apart two rows that were genuinely distinct.
  const byKey = ARCHIVE_ID_OFFSET
    ? cdb.prepare('SELECT id FROM messages WHERE sent_at = ? AND type IS ? AND (src IS NULL OR ? IS NULL OR src = ?) LIMIT 1')
    : null;
  let inserted = 0;
  const collisions = [];
  for (const it of items) {
    ...
    if (ARCHIVE_ID_OFFSET) {
      ...
      // sweep on this machine)? Never insert a duplicate — but still let a re-sweep
      // FILL late metadata and apply an enrichment upgrade to the row we already
      // have. A primary device (offset 0) gets these via the fill/upgrade below on
      // every re-sweep; without doing it here a secondary device never would, so an
      // already-archived photo/quote/link would be stuck at its original body
      // forever. Otherwise store the new arrival in this device's disjoint offset
      // band so it can't collide with copied ids.
      const hit = byKey.get(it.sentAt, it.type ?? null, it.src ?? null, it.src ?? null);
      if (hit) {
        if (it.type != null) fill.run(it.src ?? null, it.type, hit.id);
        if (it.enriched) upgrade.run(it.body, hit.id, it.body, it.body, it.body);
        if (it.attHashes) fillHashes.run(it.attHashes, hit.id);
        continue;
      }
    ...
    const r = stmt.run(id, it.convId ?? null, it.conversation ?? null, it.slug ?? null, it.sentAt, it.sender, it.body, it.src ?? null, it.type ?? null, it.attHashes ?? null);
    ...
    if (it.attHashes) fillHashes.run(it.attHashes, id);
  }
  return { seen: items.length, inserted, collisions };
}

// Record that `messageIds` were merged into `slug`'s profile (the ingest frontier).
// Called by crm-daily AFTER a chunk's merge commits, so a crash re-merges that chunk
// rather than losing it — the same crash-safety the old rowid cursor gave, but as an
// explicit per-message set so sent_at-ordered chronological ingest stays lossless.
function markMerged(cdb, slug, messageIds, at, opts = {}) {
  if (!messageIds || !messageIds.length) return;
  ensureMessagesTable(cdb);
  const ins = cdb.prepare('INSERT OR IGNORE INTO merged (slug, message_id, merged_at) VALUES (?, ?, ?)');
  const ownTransaction = opts.transaction !== false;
  if (ownTransaction) cdb.exec('BEGIN');
  try {
    for (const id of messageIds) ins.run(slug, id, at ?? null);
    if (ownTransaction) cdb.exec('COMMIT');
  } catch (e) {
    if (ownTransaction) cdb.exec('ROLLBACK');
    throw e;
  }
}
...
module.exports = { ensureMessagesTable, mirrorMessages, markMerged, extractCitations, validateCitations, CITE_RE, SYNTH_BAND };
