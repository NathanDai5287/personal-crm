...
function mirrorMessages(cdb, items) {
  ...
  // ATTACHMENT UPGRADE (the one sanctioned exception to append-only bodies): a
  // captioned photo archived before lib/attachments.js existed holds just the
  // caption, and INSERT OR IGNORE would leave it that way forever. When a writer
  // re-sees such a row and now has an "[photo] …" prefix for it, prepend it.
  // Strictly additive — it only fires when the new body ENDS WITH the stored one
  // and is longer, so real message text can never be rewritten or lost.
  ...
  for (const it of items) {
    ...
    if (it.hasMedia) upgrade.run(it.body, it.id, it.body, it.body, it.body);
    n++;
  }
  return n;
}
