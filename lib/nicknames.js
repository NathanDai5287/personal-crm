...
function openNicknamesDb() {
  ...
  // Same reasoning as openCrmDb, and the same exposure: this handle is a long-lived
  // singleton in the web app while crm-merge/crm-refresh write nicknames from the
  // pipeline — the exact reader-blocks-writer pattern that WAL removes. busy_timeout
  // absorbs writer-vs-writer; WAL removes reader-vs-writer; synchronous=FULL keeps
  // durability. Re-asserted every open so a restore heals (see lib/signal-db.js).
  // The singleton is safe under WAL: node:sqlite is autocommit between statements,
  // so it never pins a read snapshot open to starve checkpoints.
...
function proposeNickname(slug, text, messageIds, now = Date.now()) {
  ...
  try {
    ...
    // citations. ASSIGNED rows only (slug != null): an unassigned suggestion has no
    // owner to confirm it under, and auto-confirming one would make it vanish from
    // the inbox (listSuggestions is status='suggested') yet show nowhere — confirmById
    // rightly refuses NULL slugs, and this must match it. The count is of DISTINCT
    // messages (nickname_cite is unique per (nickname, message)), so re-seeing the
    // same message never inflates it, and a range/hallucinated cite can't either now
    // that parseNicknameProposals rejects ranges and cites are ledger-checked.
    const row = slug != null ? db.prepare('SELECT status FROM nickname WHERE id = ?').get(id) : null;
...
// Remove any denylist row for a (contact, name) pair — called on an EXPLICIT accept
// (hand-add or assign) so a prior dismissal can't keep the model's future cites from
// attaching to a nickname Nathan has since said he wants (P1 #4). NULL slug allowed.
function clearDenylist(db, slug, text) {
  db.prepare('DELETE FROM nickname_rejected WHERE slug IS ? AND lower(text) = lower(?)').run(slug ?? null, text);
}

// Hand-added nickname — confirmed on arrival. If one already exists for this contact
// (case-insensitively), it is a no-op that returns the existing id. Does NOT consult
// the denylist (a hand-add is a deliberate override) and CLEARS any denylist row for
// it, so re-dismissed-then-hand-added names accept the model's future cites. `created_at`
// and `confirmed_at` are both stamped so ordering stays stable.
function addNickname(slug, text, now = Date.now()) {
  const t = normText(text);
  if (!t) return null;
  const db = openNicknamesDb();
  const existing = db.prepare('SELECT id FROM nickname WHERE slug IS ? AND lower(text) = lower(?)').get(slug ?? null, t);
  if (existing) { clearDenylist(db, slug, t); return existing.id; }
  db.exec('BEGIN IMMEDIATE');
  try {
    const r = db.prepare(
...
function assignNickname(id, toSlug, now = Date.now()) {
  ...
  if (!slug) return null;
  const db = openNicknamesDb();
  const row = db.prepare('SELECT id, slug, status, text FROM nickname WHERE id = ?').get(id);
  if (!row) return null;
  // SCOPED (P1 #3): assign is an inbox triage action on an UNASSIGNED suggestion (or a
  // still-suggested row). Refuse to move an already-CONFIRMED, contact-assigned
  // nickname to a different person via this path — that's not what the inbox does, and
  // it'd let a crafted id silently relocate a confirmed name.
  if (row.slug != null && row.status === 'confirmed') return null;
  db.exec('BEGIN IMMEDIATE');
  try {
    ...
    // Explicit accept clears any denylist row for this (contact, name) — Nathan just
    // said he DOES want it, so a prior dismissal must not keep blocking future cites.
    clearDenylist(db, slug, row.text);
    db.exec('COMMIT');
    return keep;
  } catch (e) { db.exec('ROLLBACK'); throw e; }
}
...
// Two line shapes, so the model can (once the prompt allows it) say WHO a nickname
// is for:  `text | ids`  or  `target | text | ids`. The nickname text must not
// contain a `|`, so the LAST field is always the ids and a 3-field line carries a
// target name in field 0. Returns [{ target|null, text, messageIds:[<int>...] }].
function parseNicknameProposals(modelText) {
  ...
  for (const block of s.matchAll(/\[\[NICKNAMES\]\]([\s\S]*?)\[\[\/NICKNAMES\]\]/g)) {
    for (const rawLine of block[1].split(/\r?\n/)) {
      ...
      // Only the LAST field carries ids. Each id must be its OWN whitespace token and
      // exactly one m-number — a RANGE like `⟨m89123-m89130⟩` is rejected, not read as
      // two cites (that one-liner used to hit the 2-cite auto-confirm). ⟨…⟩ optional.
      const ids = [];
      for (const tok of parts[parts.length - 1].split(/\s+/)) {
        const m = tok.match(/^⟨?m(\d+)⟩?$/i);
        if (m) ids.push(parseInt(m[1], 10));
      }
      if (!ids.length) continue;
      const target = parts.length >= 3 ? (parts[0] || null) : null;
      const text = parts.length >= 3 ? parts[1] : parts[0];
      if (!text) continue;
      out.push({ target, text, messageIds: ids });
    }
  }
  return out;
}
...
// (a denylisted name proposes to null and is not counted). `opts.resolve(name)` maps a
// target name → a contact slug (or null); a named target files under the resolved
// contact, or UNASSIGNED (null slug → inbox) if it can't be resolved; no target
// defaults to `slug` (the contact being merged). `opts.validIds` (a Set/array), when
// given, is the set of ids that actually appear in this chunk's ledger: cites not in
// it are dropped and a proposal left with no real cite is skipped — so a HALLUCINATED
// id can't confirm a fabricated nickname. `opts` may be a number (back-compat `now`).
function storeNicknameProposals(slug, modelText, opts = {}) {
  const o = typeof opts === 'number' ? { now: opts } : opts;
  const now = o.now || Date.now();
  const resolve = o.resolve || null;
  const valid = o.validIds != null ? new Set([...o.validIds].map(Number)) : null;
  let n = 0;
  for (const p of parseNicknameProposals(modelText)) {
    const ids = valid ? p.messageIds.filter((id) => valid.has(Number(id))) : p.messageIds;
    if (!ids.length) continue; // every cited id was fabricated / outside the ledger
    let target = slug; // default: the contact being merged
    if (p.target) target = resolve ? (resolve(p.target) || null) : null; // named someone → resolve, else inbox
    if (proposeNickname(target, p.text, ids, now) != null) n += 1;
  }
  return n;
}
