...
function parseNicknameProposals(modelText) {
  ...
  for (const block of s.matchAll(/\[\[NICKNAMES\]\]([\s\S]*?)\[\[\/NICKNAMES\]\]/g)) {
    for (const rawLine of block[1].split(/\r?\n/)) {
      ...
      // Only the LAST field is scanned for ids, so an `m1`-looking nickname can never
      // be misread as an id. ⟨…⟩ brackets fall away naturally.
      const ids = (parts[parts.length - 1].match(/m\d+/gi) || []).map((tok) => parseInt(tok.slice(1), 10));
...
// target name → a contact slug (or null); when a line names a target, the nickname is
// filed under the resolved contact, or UNASSIGNED (null slug → the inbox) if it can't
// be resolved. With no target it defaults to `slug` (the contact being merged), the
// original behavior. `opts` may be a number for back-compat (the old `now` arg).
function storeNicknameProposals(slug, modelText, opts = {}) {
  ...
  for (const p of parseNicknameProposals(modelText)) {
    ...
    if (proposeNickname(target, p.text, p.messageIds, now) != null) n += 1;
  }
  return n;
}
