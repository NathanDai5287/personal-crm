...
// The keys a model can be picked for: the model-spending jobs (ingest, todo) plus 'self'
// — Nathan's own profile (lib/self.js), which runs inside ingest on its OWN model (merge
// and Timeline). 'self' is not a job and has no default: unset means the self pass is off.
const JOBS = [...MODEL_JOBS, 'self'];

// Curated selectable models. `paid` = separately metered per-token billing
// (moonshotai/*, deepseek/*); anthropic/* ride the Claude subscription but pi still reports a
// real dollar cost, which the UI now shows. Keep this list short and meaningful —
// it is the dropdown the user picks from.
const MODELS = [
  ...
  // Half price off-peak (see lib/cost.js rateFactor); the Monday 04:00-Pacific ingest
  // timer lands in off-peak, so scheduled runs bill at the lower rate.
  { id: 'deepseek/deepseek-flash', label: 'DeepSeek V4.1 Flash', paid: true },
];
