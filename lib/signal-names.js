...
//   3. Signal profile name    — the name THEY set on their own account
//   4. Phone number (e164)    — no name anywhere; a number beats a bare id
//   5. serviceId prefix       — last resort, so a person always has SOME handle
//
// The profile name (their own) sits at #3 — a real self-chosen name is a better
// display than a phone number. It only ever wins when you have set neither a
// nickname nor an iPhone contact for them, so it can never override your own choice
// (that was the "fingersix"/"A Ch" problem — those people now have nicknames, which
// win). Updated 2026-09-03 from "profile name ignored entirely" once the tracked
// set showed the alternative to a profile name was a bare phone number.
//
...
// Pull the candidate names out of a conversation row. `json` carries the fields
// Signal Desktop does not promote to columns; the `name`/`profile*` columns mirror
// some of them and are kept as fallbacks for older rows whose json lacks them.
function namesFromRow(row) {
  ...
  // A bare column value can carry surrounding whitespace (or be whitespace-only);
  // `col` trims it and treats empty as absent, so a name never lands with stray
  // spaces (which desyncs syncContactNames' idempotence check and would rewrite the
  // heading every sweep) or as a blank " " label. `join` already trims its result.
  const col = (v) => { const s = v == null ? '' : String(v).trim(); return s || null; };
  return {
    nickname: join(j.nicknameGivenName, j.nicknameFamilyName),
    contact: join(j.systemGivenName, j.systemFamilyName) || col(row.name),
    profile: col(row.profileFullName) || join(j.profileName, j.profileFamilyName)
      || col(row.profileName) || col(j.profileName),
    e164: col(row.e164),
  };
}
...
function nameFor(row) {
  ...
  if (n.profile) return { name: n.profile, source: 'profile name' };
...
const SELECT = `SELECT id, serviceId, name, e164, profileFullName, profileName, json
                FROM conversations
...
// labels message senders builds once per run. A person with NO name at all (no
// nickname, no contact, no phone) is deliberately LEFT OUT — the map returns
// undefined for them so the caller's own fallback ('Someone' for a third party in a
// group, the crm.db name for a tracked contact) wins. Emitting a raw serviceId here
// would freeze ugly hex ids into archive sender labels.
function signalNameMap(sdb) {
  const m = new Map();
  for (const row of sdb.prepare(SELECT).all()) {
    if (SKIP.has(row.serviceId)) continue;
    const { name, source } = nameFor(row);
    if (source === 'serviceId') continue; // no real name — let the caller's fallback decide
    m.set(row.serviceId, name);
  }
  return m;
}
