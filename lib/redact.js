...
// DETECTOR candidates — the broader known-slur set. Used by detect() ONLY to classify
// an ambiguous pi error as a content-filter rejection (crm-merge: if the error text is
// unreadable but the ledger holds a known slur, treat it as content_filter and hold the
// contact). We no longer auto-mask from this list — Nathan picks the replacement wording
// himself (learn() remains for manual/CLI use but crm-merge does not call it). `sub:true`
// matches as a substring; `sub:false` only as a whole word (so "spic" flags but not "spice").
const CANDIDATES = [
...
function saveRules(rules) {
  try { fs.mkdirSync(DATA_DIR, { recursive: true }); writeJsonAtomic(RULES_FILE, rules); } catch { /* best-effort */ }
}
