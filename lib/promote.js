...
function saveTracked(t) { fs.writeFileSync(TRACKED, `${JSON.stringify(t, null, 2)}\n`); }
...
function promoteOne(cdb, sdb, serviceId) {
  ...
  if (existing && existing.file_path) {
    ...
    while (used.has(slug) || fs.existsSync(`${CONTACTS_DIR}/${slug}.md`)) slug += '-2';
    ...
    cdb.prepare(`INSERT INTO contacts (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`).run(...cols.map((k) => vals[k]));
  }
  const absPath = `${CONTACTS_DIR}/${slug}.md`;
  if (!fs.existsSync(absPath)) fs.writeFileSync(absPath, profileTemplate(name, serviceId, conv.e164));
