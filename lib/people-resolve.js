...
// Build a resolver scoped to the TRACKED people (lib/person). This is the ONE place the
// app turns "who is tracked" into a name/nickname matcher, so every surface — the mention
// scan, the refresh cast-of-characters, the merge nickname targets, the daily resolver —
// resolves identically and an untracked stub is never matched. Inline require avoids any
// load-order coupling with lib/person.
function trackedResolver(cdb) {
  const { trackedContacts } = require('./person');
  return buildResolver(trackedContacts(cdb).map((c) => ({ slug: c.slug, name: c.name })));
}

module.exports = { buildResolver, trackedResolver };
