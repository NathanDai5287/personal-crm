...
function buildResolver(contacts = []) {
  ...
  // The OWNER (Nathan) is the reader, not a tracked contact, but the model can now
  // target a nickname at him ("Nathan | Wayne | ids"). Seed him so those resolve to
  // the 'nathan' slug (the /me store) instead of the unassigned tray — by the literal
  // "nathan" and by any nickname already confirmed for him. Not "me": too common a
  // word to scan text for. `nameBySlug` gains him so digests/labels can name him.
  built.index.set(OWNER, new Set([OWNER]));
  for (const nk of confirmedNicknames(OWNER)) {
    const key = String(nk).toLowerCase().trim();
    if (key && !built.index.has(key)) built.index.set(key, new Set([OWNER]));
  }
