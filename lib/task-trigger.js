...
function findTriggers(ledgerText, { before = BEFORE, after = AFTER, owner = 'Nathan' } = {}) {
  ...
  const windows = hits.map(({ i, msg }) => {
    ...
    for (let j = lo; j <= hi; j += 1) {
      const x = msgs[j];
      lines.push(`${j === i ? '>>> ' : '    '}[${x.when}] ⟨m${x.id}⟩ ${x.sender}: ${x.body}`);
    }
    return { msgId: msg.id, when: msg.when, body: msg.body, text: lines.join('\n') };
  });
...
function renderWindows(windows) {
  return windows
    .map((w, n) => `--- trigger ${n + 1} of ${windows.length} · ⟨m${w.msgId}⟩ ---\n${w.text}`)
    .join('\n\n');
}

module.exports = { isTrigger, isNearMiss, findTriggers, renderWindows, parseLedger, SELF, OTHER };
