...
function ownWords(body) {
  ...
  // Also strip TRAILING machine media-fold markers. These are not the person's typed
  // words, so a "make sure"/"eod" shown in a photo, spoken in a voice/video note, OR
  // described by the image CAPTION model must NEVER fire the trigger — a task has to be
  // TYPED. Covers every shape lib/media.foldSuffix emits: [image text: …] and the new
  // [image: <caption>] (photo), [transcript: "…"] (audio), [video transcript: "…"] and
  // the new [video: transcript: "…"; scenes: …] (video). Order matters — the more
  // specific "image text"/"video transcript" must precede the bare "image"/"video" so
  // they aren't half-matched. (Fold content carries no `]` — media.sanitizeFold strips
  // it — so `[^\]]*` closes on the real bracket.)
  s = s.replace(/(?:\s*\[(?:image text|video transcript|image|video|transcript):[^\]]*\])+\s*$/i, '');
  return s;
}
...
module.exports = { ownWords, isTrigger, isNearMiss, findTriggers, renderWindows, parseLedger, weekdayOf, SELF, OTHER, KEYWORDS };
