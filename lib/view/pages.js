...
function DeskActions() {
  return h('div', { class: 'jobs' },
    ...
    JobControl('Compact', 'compact', true,
      null, 'Summarize aged timeline messages into short dated lines. Calls the model.'),
...
const LEGEND_JOBS = [
  ...
  ['compact', 'Shortens aged timeline messages into one dated line each, to keep profiles readable.', 'calls the model', true, 'weekly · after ingest'],
];
const LEGEND_WORDS = [
...
/* ---- /admin/runs --------------------------------------------------------- */
// The job type IS the row's colour: sweep (free, cool blue), ingest and compact
// (both spend the model — oxblood and amber). The chip word is the pass's
// past-tense verb. A failed run (ok:false) reddens its note.
const KIND_WORD = { sweep: 'swept', ingest: 'ingested', compact: 'compacted' };
