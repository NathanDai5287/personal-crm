...
// The paid jobs that carry a per-dial pause switch (compact has no dial of its
// own — it rides the weekly ingest clock — so only ingest/todo show a switch).
const TOGGLE_SET = new Set(['ingest', 'todo', 'compact']);
// The jobs that expose a model dropdown on their card. Compact has no dial — the
// ingest card's model already governs the Timeline step server-side.
const MODEL_SET = new Set(['ingest', 'todo']);

function RunDial(d, toggles = {}, models = {}, modelOptions = []) {
  ...
  return h('div', { class: `dial${d.kind ? ' ' + d.kind : ''}${d.overdue && !paused ? ' overdue' : ''}${paused ? ' paused' : ''}` },
    ...
    h('div', { class: 'dialcap' },
      ...
      // form to /admin/model. Changing the select auto-submits where JS is on; the
      // "set" button is the no-JS fallback. "default" (value "") clears the override.
      MODEL_SET.has(d.job) ? h('div', { class: 'dialmodel' },
        h('select', {
          ...
          h('option', { value: '', selected: !models[d.job] ? true : undefined }, 'default'),
          modelOptions.map((m) => h('option', {
            value: m.id, selected: models[d.job] === m.id ? true : undefined,
          }, `${m.label}${m.paid ? ' · paid' : ''}`))),
        h('button', {
          type: 'submit', name: 'setmodel', value: d.job,
          formaction: '/admin/model', formmethod: 'post', class: 'dmset btn sm',
        }, 'set')) : null));
}
...
function admin({ health, roster, dials, toggles, models, modelOptions }) {
  const body = h('div', {},
    h('div', { class: 'mast' },
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      Head('Cadence', 'each dial is a job · the arc is time elapsed, the number is time until next · press ▸ run to trigger it · the top-right switch pauses a paid dial’s automatic schedule · the model dropdown picks which LLM it uses'),
      h('div', { class: 'dials' }, dials.map((d) => RunDial(d, toggles || {}, models || {}, modelOptions || []))),
...
const KIND_WORD = { sweep: 'sweep', ingest: 'ingest', compact: 'timeline', todo: 'todo', manual: 'edit' };
...
function RunsFilter(records) {
  ...
  return h('div', { class: 'runfilter' },
    ...
    h('label', { class: 'rffree' },
      h('input', { type: 'checkbox', id: 'rfHideFree' }), ' hide free runs'));
}
