...
function RosterRow(x) {
  ...
  return h('tr', {},
    ...
    h('td', { class: 'r' }, x.last),
    h('td', {},
      jobButton('sweep', `sweep:${x.slug}`, `Sweep ${x.name} now? No model calls.`, 'sm'), ' ',
      jobButton('ingest', `ingest:${x.slug}`, `Ingest ${x.name}? Reads their archived messages into the profile. Calls the model.`, 'sm pr')));
}
...
// trigger. Clicking submits the form, which carries every checked roster slug
// (name="who") plus any modifier. Confirms first; a cancel returns false and the
// form is not submitted.
function jobButton(label, value, message, cls) {
  const safe = String(message).replace(/'/g, "\\'");
  return h('button', { type: 'submit', class: `btn ${cls || ''}`, name: 'job', value,
    onclick: `return confirm('${safe}')` }, label);
}

// `modifier` is null or { name, label }: a named checkbox the server reads for
// this kind only (deep for sweep, plan for ingest).
function JobControl(label, value, message, primary, modifier, desc) {
  return h('div', { class: 'jobrow' },
    jobButton(label, value, message, primary ? 'pr' : ''),
    modifier
...
function DeskActions() {
  return h('div', { class: 'jobs' },
    JobControl('Sweep', 'sweep', 'Run sweep now? Acts on the checked people, or everyone if none are checked. No model calls.', true,
      { name: 'deep', label: 'deep' }, 'Copy new messages from Signal into the archive. Free, no model.'),
    JobControl('Ingest', 'ingest', 'Run ingest now? Acts on the checked people, or everyone if none are checked. This calls the model.', false,
      { name: 'plan', label: 'plan only' }, 'Read archived messages into profiles, week by week, with citations. Calls the model.'),
    JobControl('Compact', 'compact', 'Run compact now? Acts on the checked people, or everyone if none are checked. Calls the model.', false,
