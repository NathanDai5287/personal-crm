...
function admin({ health, roster, dials, toggles }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      Head('Cadence', 'each dial is a job · the arc is time elapsed, the number is time until next · press ▸ run to trigger it · the top-right switch on a paid dial pauses its automatic schedule (a hand-started ▸ run still works)'),
      h('div', { class: 'dials' }, dials.map((d) => RunDial(d, toggles || {}))),
