...
const NAV = [['/', 'People'], ['/tasks', 'To do'], ['/graph', 'Graph'], ['/admin', 'Pipeline'], ['/admin/runs', 'Runs'], ['/health', 'Health'], ['/me', 'Me']];
...
// hasProfile: data/contacts/nathan.md exists (the self profile, lib/self.js) — link it.
function me(nicks, hasProfile = false) {
  const body = h('div', {},
    h('div', { class: 'mast' },
    ...
    h('div', { class: 'profile' },
      ...
      hasProfile ? h('p', {}, h('a', { href: profilePath('nathan') }, 'Your own profile →')) : null));
...
function ProgressBar(x) {
  ...
  const tip = [
    label,
    waiting > 0 ? `${waiting.toLocaleString()} waiting` : null,
    isQuiet(x) ? 'quiet' : null,
    x.last ? `last ${x.last}` : null,
    x.heldReview ? 'held for censor review' : null,
  ].filter(Boolean).join(' · ');
  return h('div', { class: 'pcell' },
    ...
    h('div', { class: 'ptip', role: 'tooltip' },
      x.heldReview
        ? h('span', { class: 'held-badge' }, 'held · review')
...
// The self row's model picker: which model runs Nathan's own profile (merge + Timeline).
// Same nested-form dodge as the dial pickers (a submit button named setmodel posts the
// desk form to /admin/model). Unlike them it has an "off" choice, and off is the
// default: the self pass's first run is a whole-archive backfill.
function SelfModelPick(x) {
  return h('span', { class: 'dialmodel rowmodel' },
    h('select', {
      name: 'model:self', class: 'dmsel', 'aria-label': 'model for Nathan’s own profile',
      title: 'model for your own profile (merge + Timeline); off = not built',
      onchange: "var b=this.parentNode.querySelector('.dmset'); if(b) b.click();",
    },
    h('option', { value: '', selected: x.selfModel ? undefined : true }, 'self: off'),
    (x.modelOptions || []).map((m) => h('option', {
      value: m.id, selected: x.selfModel === m.id ? true : undefined,
    }, m.label))),
    h('button', { type: 'submit', name: 'setmodel', value: 'self', formaction: '/admin/model', formmethod: 'post', class: 'dmset' }, 'set'));
}
...
function RosterRow(x) {
  return h('tr', x.heldReview ? { class: 'held-row' } : {},
    ...
    h('td', { class: 'rowacts' },
      // Nathan's own row has no sweep (his messages ARE every contact's) — its slot holds
      // the self-model picker instead.
      x.self ? SelfModelPick(x) : jobButton('sweep', `sweep:${x.slug}`, 'sm'), ' ',
...
function admin({ health, roster, dials, toggles, models, modelOptions, modelDefaults }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      ...
      Head('Roster', `all ${roster.length} · check people to scope a run to them (else everyone) · a row’s own trigger runs just one · names come from Signal`),
      RosterTable(roster)),
    Legend(),
...
function ProgressBlock(j) {
  ...
  return h('div', { class: 'prog', style: 'margin:12px 0' },
    h('div', { class: 'jobbar', id: 'pbar', 'data-phase': phase },
...
module.exports = { Tabs, Head, Slip, Nicks, NnInbox, people, profile, me, tasks, admin, runs, message, job };
