...
// Only the PAID jobs carry a per-dial pause switch — the free sweeps always run
...
function admin({ health, roster, dials, toggles, models, modelOptions }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      Head('Cadence', 'each dial is a job · the arc is time elapsed, the number is time until next · press ▸ run to trigger it · paid jobs (ingest · todo) carry a switch that pauses their automatic schedule and a dropdown for the model · the free sweeps always run'),
