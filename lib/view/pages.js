...
function TaskSource(t) {
  return h('div', { class: 'tasksrc' },
    ...
    t.msgId ? [' · agreed in ', Slip({ a: t.msgId })] : '',
    t.owner && t.owner !== 'nathan' ? ` · owner ${t.owner}` : '',
    t.probable ? [' · ', h('span', { class: 'prob' }, 'probable')] : '');
}

function EditForm(t) {
  return h('form', { method: 'post', action: '/tasks/edit', class: 'tedit' },
    h('input', { type: 'hidden', name: 'id', value: t.id }),
    h('label', {}, 'Title', h('input', { class: 'fld', name: 'title', value: t.title, maxlength: '300' })),
    h('label', {}, 'Description', h('textarea', { class: 'fld', name: 'description', rows: '2' }, t.description || '')),
    h('label', {}, 'Deadline ', h('span', { class: 'hint' }, 'eod · tomorrow · monday · aug 15 · a date'),
      h('input', { class: 'fld tw', name: 'deadline', value: t.deadline || '', placeholder: 'eod, monday, aug 15…', maxlength: '30' })),
    h('label', {}, 'Priority', ImpSelect(t.importance)),
    h('div', { class: 'tedit-acts' },
      h('button', { type: 'submit', class: 'btn pr' }, 'Save'),
      h('a', { class: 'btn', href: '/tasks' }, 'Cancel')));
}
...
// aside so a "HIGH" stamp never shifts the title, and the edit link is pinned to
// the bottom-left of every card regardless of what else the card holds.
function TaskCard(t, editing, today) {
  const done = t.status === 'done';
  const overdue = t.deadline && t.deadline < today && !done;
  const isEditing = editing && editing.id === t.id;
  return h('div', { class: `taskcard${done ? ' done' : ''}` },
    ...
    h('div', { class: 'taskbody' },
      h('div', { class: 'tasktitle' }, t.title),
      t.descHtml ? h('div', { class: 'taskdesc' }, raw(t.descHtml)) : '',
      TaskSource(t),
      isEditing
        ? EditForm(t)
        : h('div', { class: 'taskfoot' },
            h('a', { class: 'btn sm', href: `/tasks?edit=${t.id}` }, 'edit'),
    ...
    h('div', { class: 'taskaside' },
      t.deadline ? h('div', { class: `taskdue${overdue ? ' od' : ''}` }, h('span', {}, 'due'), t.deadline) : '',
      ImpMark(t.importance)));
}
...
function tasks({ counts, active, done, drafts, editing, today }) {
  ...
  const body = h('div', {},
    ...
    h('div', { class: 'todo' },
      h('div', {},
        ...
        h('div', { id: 'activeList' },
          active.map((t) => TaskCard(t, editing, today)),
        ...
        h('div', { id: 'doneWrap', style: doneCount ? undefined : 'display:none' },
          ...
          h('div', { id: 'doneList' }, done.map((t) => TaskCard(t, editing, today))))),
      h('div', { class: 'intray' },
...
function RunDial(d) {
  ...
  return h('div', { class: `dial${d.kind ? ' ' + d.kind : ''}${d.overdue ? ' overdue' : ''}` },
...
// the free/paid split is legible at a glance.
...
const LEGEND_JOBS = [
  ['sweep', 'sweep', 'Copies new messages from Signal into the archive, so nothing is lost — including disappearing messages.', 'free · no model', false, 'hourly · deep daily'],
  ...
  ['compact', 'timeline', 'Builds and maintains the chronological Timeline — recent messages raw, older ones aged into one dated line per day, then per week.', 'calls the model', true, 'weekly · after ingest'],
  ['todo', 'todo', 'Scans new messages for "make sure …" commitments and drafts a task from each; a model runs only when a line matches.', 'model only on a match', false, 'hourly · after sweep'],
];
const LEGEND_WORDS = [
  ...
  ['cursor', 'How far ingest has read into a person’s archive. Everything past it is still waiting.'],
  ['waiting', 'Archived messages ingest has not folded into the profile yet.'],
...
function admin({ health, roster, dials }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      Head('Cadence', 'each dial is a job · the arc is time elapsed, the number is time until next · press ▸ run to trigger it'),
...
/* ---- /admin/runs --------------------------------------------------------- */
// The job type IS the row's colour: sweep (free, cool blue), ingest and timeline
// (both spend the model — oxblood and amber). A failed run (ok:false) reddens its
// note. `kind` stays the internal job id ('sweep'|'ingest'|'compact'); only the
// shown word changed when Compact was relabelled Timeline.
const KIND_WORD = { sweep: 'swept', ingest: 'ingested', compact: 'timeline', todo: 'scanned', manual: 'by hand' };
const KIND_LABEL = { sweep: 'Sweep', 'deep-sweep': 'Deep sweep', ingest: 'Ingest', compact: 'Timeline', todo: 'Todo' };
const NUMERIC_COLS = new Set(['examined', 'held', 'est', 'actual', 'took']);
const isFreeMoney = (v) => v === 'free' || v === '$0 · sub' || v === '$0';

function RunRow(r) {
  // The `when` cell links a completed row to its detail (steps + log), a live row
  // to the monitor. Live rows also link their note ("monitor live →").
  const whenCell = r.href ? h('td', { class: 'tw' }, h('a', { href: r.href }, r.t)) : h('td', { class: 'tw' }, r.t);
  const noteCell = r.live && r.href
    ? h('td', { class: 'hand livenote' }, h('a', { href: r.href }, r.note))
    : h('td', { class: r.ok === false ? 'hand bad' : 'hand' }, r.note);
  return h('tr', { class: r.live ? 'runrow live' : 'runrow' },
    whenCell,
    h('td', {}, Mk(r.kind, KIND_WORD[r.kind] || r.kind)),
    h('td', {}, r.pass), h('td', {}, r.scope),
    h('td', { class: 'r' }, r.examined), h('td', { class: 'r' }, r.held),
    h('td', { class: `r cost est${isFreeMoney(r.cost) ? ' free' : ''}` }, r.cost),
    h('td', { class: `r cost${isFreeMoney(r.actual) ? ' free' : r.actual === '—' ? ' none' : ''}` }, r.actual),
    h('td', { class: 'r' }, r.took),
    noteCell);
}

function runs(records) {
  const cols = ['when', 'kind', 'pass', 'scope', 'examined', 'held', 'est', 'actual', 'took', 'note'];
  const body = h('div', {},
    h('div', { class: 'mast' },
...
// ETA); an indeterminate pulse during the Timeline (compaction) phase, whose
...
/* ---- /c/<slug>/rename ---------------------------------------------------- */
/* ---- /c/<slug>/fields — hand-set header fields ---------------------------- */
// Only Relationship and Birthday: they are Nathan's knowledge, not something the
// pipeline can derive. Every other metadata line (ids, phone, contact dates,
// message counts) is machine-owned and stays out of the form. A save lands in
// the runs ledger as a 'manual' pass, one run per profile per save.
function fields({ slug, name, relationship, birthday }) {
  const body = h('div', {},
    Tabs('/'),
    h('div', { class: 'back' }, h('a', { href: `/c/${slug}` }, `← ${name}`)),
    h('div', { class: 'profile', style: 'max-width:540px' },
      h('h1', {}, 'Edit fields'),
      h('p', { class: 'rel' }, 'Hand-set the header fields for ', h('code', {}, slug),
        '. Leave one blank to reset it to TBD. Each save is logged in the runs ledger as a manual pass.'),
      h('form', { method: 'post', action: `/c/${slug}/fields`, class: 'tedit' },
        h('label', {}, 'Relationship',
          h('input', { class: 'fld', name: 'relationship', value: relationship, autofocus: true,
            placeholder: 'high school friend, coworker, roommate…', maxlength: '120' })),
        h('label', {}, 'Birthday',
          h('input', { class: 'fld', name: 'birthday', value: birthday, placeholder: 'August 31', maxlength: '120' })),
        h('label', {}, 'Why (optional)',
          h('textarea', { class: 'fld', name: 'message', rows: '2', maxlength: '500',
            placeholder: 'context for future-you — kept with the edit in the ledger and history' })),
        h('div', { class: 'tedit-acts' },
          h('button', { type: 'submit', class: 'btn pr' }, 'Save'),
          h('a', { class: 'btn', href: `/c/${slug}` }, 'Cancel')))));
  return { title: `Edit fields — ${name}`, body };
}
...
module.exports = { Tabs, Head, Slip, people, profile, tasks, admin, runs, message, job, rename, fields };
