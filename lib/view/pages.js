...
// The paid periodic jobs, each with an on/off switch. A pause stops only the
// AUTOMATIC schedule (systemd timer / Task Scheduler); a hand-started run from a
// dial passes --force and always proceeds. Each switch is its own tiny form — it
// MUST sit outside the /admin/jobs form (no nested forms), so it lives here above
// the Cadence section.
const TOGGLE_JOBS = [
  ['ingest', 'Ingest', 'weekly merge + Timeline'],
  ['todo', 'Todo capture', 'hourly “make sure” scan'],
  ['compact', 'Compact / Timeline', 'standalone Timeline summaries'],
];
function TogglesPanel(toggles) {
  return h('div', { class: 'toggles' }, TOGGLE_JOBS.map(([job, label, sub]) => {
    const on = toggles[job] !== false;
    return h('form', { method: 'post', action: '/admin/toggle', class: `toggle ${on ? 'on' : 'off'}` },
      h('input', { type: 'hidden', name: 'job', value: job }),
      h('input', { type: 'hidden', name: 'enabled', value: on ? '0' : '1' }),
      h('span', { class: 'toggle-main' }, h('b', {}, label), h('span', { class: 'toggle-sub' }, sub)),
      h('span', { class: `toggle-state ${on ? 'on' : 'off'}` }, on ? 'On' : 'Paused'),
      h('button', { type: 'submit', class: 'btn sm' }, on ? 'Pause' : 'Resume'));
  }));
}

function admin({ health, roster, dials, toggles }) {
  const body = h('div', {},
    h('div', { class: 'mast' },
    ...
    Head('Automatic runs', 'pause a paid job’s schedule without touching the timers · a hand-started run always overrides'),
    TogglesPanel(toggles || {}),
...
module.exports = { Tabs, Head, Slip, Nicks, people, profile, tasks, admin, runs, message, job, rename };
