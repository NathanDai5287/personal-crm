...
// Every job carries a per-dial pause switch — the four in lib/jobs.js, the single
// source of truth. (Timeline is not a job; it has no dial and no switch, because
// it rides ingest — see lib/jobs.js.) The free sweeps' switch pauses archiving.
const { JOB_IDS, MODEL_JOBS } = require('../jobs');
const TOGGLE_SET = new Set(JOB_IDS);
...
const LEGEND_JOBS = [
  ...
  ['timeline', 'timeline', 'Ingest’s second half: builds and maintains the chronological Timeline — recent messages raw, older ones aged into one dated line per day, then per week.', 'calls the model', true, 'part of ingest'],
...
function admin({ health, roster, dials, toggles, models, modelOptions }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      Head('Cadence', 'each dial is a job · the arc is time elapsed, the number is time until next · press ▸ run to trigger it · the top-right switch pauses that job’s automatic schedule · the model dropdown (paid jobs) picks which LLM it uses'),
