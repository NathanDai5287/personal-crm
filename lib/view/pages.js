...
function TaskCard(t, editing, today) {
  ...
  return h('div', { class: `taskcard${done ? ' done' : ''}` },
    ...
    h('div', { class: 'taskbody' },
      ...
      isEditing
        ? EditForm(t)
        : h('div', { class: 'taskfoot' }, h('a', { class: 'btn sm', href: `/tasks?edit=${t.id}` }, 'edit'))),
    ...
    h('div', { class: 'todo' },
      h('div', {},
        ...
        active.length
          ...
          : h('p', { class: 'emptyline' }, 'Nothing charged out. Add one above, or accept a draft from the in-tray.'),
        doneCount ? [Head('Done', `${doneCount} filed`), done.map((t) => TaskCard(t, editing, today))] : ''),
      h('div', { class: 'intray' },
...
function RosterRow(x) {
  ...
  return h('tr', {},
    h('td', { class: 'pickcell' }, h('input', { type: 'checkbox', class: 'pick', name: 'who', value: x.slug, 'aria-label': `select ${x.name}` })),
    h('td', { class: 'person' },
...
// `modifier` is null or { name, label }: a named checkbox the server reads for
// this kind only (deep for sweep, plan for ingest).
function JobControl(label, value, primary, modifier, desc) {
  return h('div', { class: 'jobrow' },
    jobButton(label, value, primary ? 'pr' : ''),
    modifier
      ? h('label', { class: 'mod' }, h('input', { type: 'checkbox', name: modifier.name }), ' ', modifier.label)
      : h('span', { class: 'mod' }),
    h('span', { class: 'jobdesc' }, desc));
}

// Oxblood (primary) marks the runs that spend the model — ingest and compact —
// so the free sweep is the plain one. Red should flag cost, not the safe copy.
function DeskActions() {
  return h('div', { class: 'jobs' },
    JobControl('Sweep', 'sweep', false,
      { name: 'deep', label: 'deep' }, 'Copy new messages from Signal into the archive. Free, no model.'),
    JobControl('Ingest', 'ingest', true,
      { name: 'plan', label: 'plan only' }, 'Read archived messages into profiles, week by week, with citations. Calls the model.'),
    JobControl('Timeline', 'compact', true,
      null, 'Build and maintain the chronological Timeline, ageing entries from raw to daily to weekly lines. Calls the model.'),
    h('div', { class: 'jobnote' }, 'Runs act on the people checked below — or everyone if none are checked. One run at a time. Destructive resets stay on the command line.'));
}
...
const LEGEND_JOBS = [
  ['sweep', 'Copies new messages from Signal into the archive, so nothing is lost — including disappearing messages.', 'free · no model', false, 'hourly · deep daily'],
  ['ingest', 'Reads a person’s waiting messages into their profile, week by week, each claim citing a message.', 'calls the model', true, 'weekly · Mondays'],
  ['timeline', 'Builds and maintains the chronological Timeline — recent messages raw, older ones aged into one dated line per day, then per week.', 'calls the model', true, 'weekly · after ingest'],
  ['todo', 'Scans new messages for "make sure …" commitments and drafts a task from each; a model runs only when a line matches.', 'model only on a match', false, 'hourly · after sweep'],
];
const LEGEND_WORDS = [
  ...
  return h('div', {},
    ...
    h('div', { class: 'legend' },
      h('table', { class: 'legend-jobs' },
        ...
        h('tbody', {}, LEGEND_JOBS.map(([term, what, cost, model, cadence]) =>
          h('tr', {},
            h('td', {}, h('span', { class: 'legend-term' }, term)),
...
function admin({ health, roster, dials }) {
  const body = h('div', {},
    ...
    h('form', { method: 'post', action: '/admin/jobs' },
      ...
      Head('Roster', `all ${roster.length} · check people then run above, or use a row’s own trigger · ✎ renames`),
      RosterTable(roster)),
    Legend(),
...
const KIND_WORD = { sweep: 'swept', ingest: 'ingested', compact: 'timeline', todo: 'scanned' };
const KIND_LABEL = { sweep: 'Sweep', ingest: 'Ingest', compact: 'Timeline', todo: 'Todo' };
...
function runs(records) {
  const cols = ['when', 'kind', 'pass', 'scope', 'examined', 'held', 'took', 'note'];
  const body = h('div', {},
    h('div', { class: 'mast' },
...
module.exports = { Tabs, Head, Slip, people, profile, tasks, admin, runs, message, job, rename };
