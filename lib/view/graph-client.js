...
(function () {
  ...
  var W = 1400, H = 950, CX = W / 2, CY = H / 2;
  ...
  // Snappy hover tooltip for edges (the native <title> lags ~1s). Positioned in
  // the stage, follows the cursor, names both people on the edge.
  var tip = document.createElement('div');
  tip.className = 'graph-tip'; tip.hidden = true;
  host.appendChild(tip);
  function showTip(e) {
    tip.textContent = name(e.from) + ' → ' + name(e.to) + ' · ' + e.n + ' mention' + (e.n === 1 ? '' : 's');
    tip.hidden = false;
  }
  function moveTip(ev) {
  ...
  function build() {
    ...
    ve.forEach(function (e) {
      // Visible curve. pointer-events go to the transparent wide `hit` path below,
      // so thin edges are still easy to hover/click.
      ...
      hit.addEventListener('pointerenter', function () { path.classList.add('hover'); showTip(e); });
      hit.addEventListener('pointermove', function (ev) { moveTip(ev); });
      hit.addEventListener('pointerleave', function () { path.classList.remove('hover'); hideTip(); });
      edgeLayer.appendChild(hit);
      edgeLayer.appendChild(path);
      edgeEls.push({ e: e, path: path, hit: hit });
    });
  ...
  // Tuned to spread the graph out: strong node-node repulsion, long spring rest
  // lengths, and a gentle centre/cohesion pull so clusters stay legible without
  // bunching up.
  var REPULSION = 6000, SPRING = 0.018, CENTER = 0.006, COHESION = 0.012, DAMP = 0.9;
  ...
  function tick() {
    ...
    for (i = 0; i < ve.length; i += 1) {
      ...
      var rest = 320 - Math.min(180, Math.log(1 + e.n) * 55);
  ...
  function render() {
    for (var i = 0; i < edgeEls.length; i += 1) {
      var d = edgePath(edgeEls[i].e);
