...
function buildMessageQuery(sources, contactServiceId, bound) {
  const { dmConvIds, biGroupConvIds, multiGroupConvIds, allIds } = sources;
  const partyIds = allIds && allIds.length ? allIds : [contactServiceId];
  const clauses = [];
  const params = [];
  ...
  if (biGroupConvIds.length) {
    clauses.push(`(conversationId IN (${inList(biGroupConvIds)}) AND (type = 'outgoing' OR sourceServiceId IN (${inList([MY_SERVICE_ID, ...partyIds])})))`);
    params.push(...biGroupConvIds, MY_SERVICE_ID, ...partyIds);
  }
  if (multiGroupConvIds.length) {
    clauses.push(`(conversationId IN (${inList(multiGroupConvIds)}) AND sourceServiceId IN (${inList(partyIds)}))`);
    params.push(...multiGroupConvIds, ...partyIds);
  }
...
function buildArchiveQuery(sources, contactServiceId, bound, opts = {}) {
  ...
  if (biGroupConvIds.length) {
    clauses.push(`(conv_id IN (${inList(biGroupConvIds)}) AND (type = 'outgoing' OR src IN (${inList([MY_SERVICE_ID, ...partyIds])})))`);
    params.push(...biGroupConvIds, MY_SERVICE_ID, ...partyIds);
  }
  if (multiGroupConvIds.length) {
    clauses.push(`(conv_id IN (${inList(multiGroupConvIds)}) AND src IN (${inList(partyIds)}))`);
    params.push(...multiGroupConvIds, ...partyIds);
  }
  if (clauses.length === 0) return null;
  ...
  // the rowid ordering is dropped then since it's meaningless for an aggregate.
